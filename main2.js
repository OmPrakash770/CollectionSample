...
const openapi = {
  ...
  async function createFolderStructure(openapi) {
    ...
    client.connect((err) => {
      ...
      // You can now execute queries using the client object
    ...
    Object.keys(paths).forEach(async (pathKey) => {
      const pathValue = paths[pathKey];
      const folderName = pathKey.replace('/', '_');
      fs.mkdirSync(folderName, { recursive: true });
  
      await client.query('INSERT INTO folders (name) VALUES ($1)', [folderName]);
  
      Object.keys(pathValue).forEach(async (method) => {
        const fileName = `${method.toUpperCase()}.js`;
        fs.writeFileSync(path.join(folderName, fileName), `// ${method.toUpperCase()} ${pathKey}`);
  
        await client.query('INSERT INTO files (name, folder_name) VALUES ($1, $2)', [fileName, folderName]);
      });
    });
